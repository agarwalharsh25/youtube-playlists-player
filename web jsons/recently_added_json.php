<?php
	...
	$select = "SELECT v.`video_id`, v.`title`, v.`playlist_id`, v.`description`, v.`publish_date`, v.`views`, v.`thumbnail`, p.`playlist`, p.`playlist_image`, p.`category_id`, c.`category` FROM `videos` v, `playlists` p, `categories` c WHERE v.playlist_id=p.id AND p.category_id=c.id ORDER BY v.`timestamp` DESC LIMIT 10";
